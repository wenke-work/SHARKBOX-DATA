...
select
    ...
    ,0                                                                                                             as Interest                        -- 利息
from
    (select @Declare_Serial_Number as Declare_Serial_Number,RateValue from dbo.VAT_rate where CountryId in(select ID from dbo.Country where CountryName='意大利')) tb0
left join
